...
Disadvantages this model might include: i) lacks a parameter corresponding directly with a critical soil test value (the model cannot be evaluated at the asymptote as it would result in a CSTV equal to `Inf`); and ii) there is no apparent confidence interval for an estimated CSTV. For this latter purpose, we recommend the user to use the `boot_mitscherlich()` function for a  reliable confidence interval estimation of parameters and CSTV via bootstrapping (resampling with replacement). The `mitscherlich()` function works automatically with self-starting initial values to facilitate the model's convergence. <br/>
...
A suitable alternative for obtaining confidence intervals for parameters of derived quantities is bootstrapping. <br/>
...
```{r}
boot_mits <- boot_mitscherlich(data = data_1, type = 1,
                    ry = RY, target = 90,
                    stv = STV, n = 200)

boot_mits %>% dplyr::slice_head(., n=5)
