...
This is used in the `mitscherlich()` function. The model can also be expressed as
...
1.  `type = 3 or "free"` (DEFAULT): three parameter model; $y = a * (1-e^{-c(x + b)})$
2.  `type = 2 or "100"`: two parameter model where asymptote = 100% RY; $y = 100 * (1-e^{-c(x + b)})$
3.  `type = 1 or "fixed":` one parameter model in which only the curvature varies and asymptote = 100 and model goes through origin; $y = 100 * (1-e^{-cx})$.
...
### 1.1. Different number of parameters `type = 3`
...
data.all %>% 
  group_by(id) %>% 
  group_modify(~ mitscherlich(data = ., STV, RY))
