...
Disadvantages this model might include: i) lacks a parameter corresponding directly with a critical soil test value (the model cannot be evaluated at the asymptote as it would result in a CSTV equal to `Inf`); and ii) there is no apparent confidence interval for an estimated CSTV. For this latter purpose, we recommend the user would to use a re-sampling technique (e.g. bootstrapping) for a  reliable confidence interval estimation of parameters and CSTV (for examples on bootstrapping, see [nlraa package vignette](https://cran.r-project.org/web/packages/nlraa/vignettes/Bootstrapping.html). The `mitscherlich()` function works automatically with self-starting initial values to facilitate the model's convergence. <br/>
