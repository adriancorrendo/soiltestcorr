...
This tutorial demonstrates the `mitscherlich()` function for fitting a continuous response model and estimating a critical soil test value. This function fits a Mitscherlich-type exponential regression model that follows a diminishing growth curve, and is sometimes also referred to as exponential "rise-to-the-max". The model is expressed in `mitscherlich()` as
...
In this formulation, the X-intercept = $-\frac{ln(-\frac{a}{b-a})}{c}$

Note that sometimes the model is expressed using alternative formulations, as in Cerrato and Blackmer (1990):
...
where\
`a` = asymptote,\
`b` = related to X-intercept, where X-intercept = -b,\
`c` = curvature parameter.
...
1.  `type = 1, "no restriction", or "free"` (DEFAULT): three parameter model; $y = a + (b - a) * e^{-cx}$
2.  `type = 2, "asymptote 100", or "100"`: two parameter model where asymptote = 100% RY; $y = 100 + (b - 100) * e^{-cx}$
3.  `type = 3, "asymptote 100 from 0", or "fixed":` one parameter model in which only the curvature varies and asymptote = 100 and model goes through origin; $y = 100 + (0 - 100) * e^{-cx}$.
...
2.  Specify the following arguments into the function `mitscherlich()`:
    1.  `type` select the type of parameterization of the model (`type = 3, 2, or 1; see above`)
    2.  `data` (optional)
